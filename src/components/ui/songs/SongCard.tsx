...
export function SongCard({ song, viewMode = 'grid' }: SongProps) {
  ...
  if (viewMode === 'list') {
    return (
      <div className="group flex w-full items-center gap-2 border-b border-accent/10 bg-background px-1 transition-colors hover:bg-muted/40 sm:gap-3 sm:px-2">
        <Link
          to="/songs/$id"
          params={{ id: song.id }}
          preload="intent"
          className="flex min-w-0 flex-1 items-center gap-3 py-2.5 active:opacity-70 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-accent sm:gap-4 sm:py-3"
        >
        ...
        <AddToSetlistButton
          ...
          iconOnly
          className="h-11 w-11 shrink-0 rounded-none border-accent/20 p-0"
        />

        <Link to="/songs/$id" params={{ id: song.id }} preload="intent" aria-label={`Open ${song.title}`} className="shrink-0 py-3 pl-0.5">
          <ChevronRight className="h-4 w-4 text-accent/50 transition-transform group-hover:translate-x-0.5" aria-hidden="true" />
        </Link>
      </div>
    );
  }
