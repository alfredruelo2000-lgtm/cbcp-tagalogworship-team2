...
function SongLibraryPage() {
  ...
  return <main className="container mx-auto px-4 sm:px-6 pb-16 pt-5 md:py-14">
    ...
    <section className="sticky top-16 z-30 -mx-4 mb-5 border-b border-accent/10 bg-background/95 px-4 py-2.5 backdrop-blur-md sm:-mx-6 sm:px-6 lg:top-20 md:mb-9 md:py-5" aria-label="Song library controls">
      ...
      <div className="scrollbar-none -mb-1 flex gap-1 overflow-x-auto pt-2">{languages.map((lang) => (counts[lang] ?? 0) > 0 && <Button key={lang} variant={languageFilter === lang ? 'secondary' : 'ghost'} className="h-9 shrink-0 rounded-none px-3 text-xs" onClick={() => setLanguageFilter(lang)}>{lang} <span className="ml-1.5 text-muted-foreground">{counts[lang] ?? 0}</span></Button>)}</div>
